...
public class ReviewMapper {
    public ReviewResponse toReviewResponse(Review review) {
        ...
        res.setIsVerified(review.getIsVerified());
        
        if (review.getCar() != null) res.setCarId(review.getCar().getId());
        if (review.getUser() != null) {
            res.setUserId(review.getUser().getId());
            res.setUserName(review.getUser().getFirstName() + " " + review.getUser().getLastName());
        }
        if (review.getBooking() != null) res.setBookingId(review.getBooking().getId());
